...
from datasets.creator import VQADatasetCreator
...
class VGGGPT2DatasetCreator(VQADatasetCreator):
...
class VGGPT2Dataset(BeamSearchDataset):
    """
    ...
    def __getitem__(self, item):
        ...
        if not self.bleu_batch:
            ...
            return identifier, sequence, image, seq_len, q_len, a_len

    def __len__(self):
        return self.maxlen

    def get_bleu_inputs(self, model, batch, device):
        answer_start_idx = batch[3][0] - batch[5][0]
        question = batch[1][0][:answer_start_idx].to(device)
        beam_search_input = BeamSearchInput(model, 0, 0, question)
        ground_truths = []
        for seq, seq_len, a_len in zip(batch[1], batch[3], batch[5]):
...
def create(tr_size=1000000, ts_size=100000):
    destination = resources_path('models', 'vgg_gpt2', 'data')
    dsc = VGGGPT2DatasetCreator(tokenizer=gpt2_tokenizer, tr_size=tr_size, ts_size=ts_size, generation_seed=555)
