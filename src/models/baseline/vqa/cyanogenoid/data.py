...
class VQA(data.Dataset):
    ...
    def max_question_length(self):
        if not hasattr(self, '_max_length'):
            self._max_length = max(map(lambda question: len(question[1]), self.questions))
    ...
    def __getitem__(self, item):
        ...
        q, q_length, __id = self.questions[item]
        a = self.answers[item]
