...
def do_beam_search(__model, question, image, beam_search_input, device='cuda', beam_size=1, maxlen=20):
    ...
    if cs is not None:
        ...
        return gpt2_tokenizer.decode(rm_eos_token(cs)), softmap_visualize(cs_out, seq, image, False)
    elif rs is not None:
        seq = torch.cat([question, torch.tensor(rs).to(device)])
        return gpt2_tokenizer.decode(rm_eos_token(rs)), softmap_visualize(rs_out, seq, image, False)
